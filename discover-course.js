...
export function createDiscoverCourseRenderers({app,state,navigate}) {
  ...
  function complete(){if(!course.completed){navigate(course.started?'#level/discover/question':'#level/discover/start');return;}app.innerHTML=`<section class="recognize-complete page-shell"><div><p class="section-label">分析完成</p><h1>第二關完成</h1><p class="recognize-complete-lead">你不只看得出來，也開始知道「為什麼」。</p><p>同一項形式原理，換了造形、位置、色彩或排列方式，你仍然能找到判斷的線索。</p><button class="primary-button" id="discover-wall">返回實驗室</button><button class="back-link discover-next-stage" id="discover-experiment">第三關｜製作實驗</button></div><div class="discover-complete-art" aria-hidden="true"><i></i><i></i><i></i><i></i><i></i></div></section>`;document.querySelector('#discover-wall').addEventListener('click',()=>navigate('#principles'));document.querySelector('#discover-experiment').addEventListener('click',()=>navigate('#level/experiment/start'));}
